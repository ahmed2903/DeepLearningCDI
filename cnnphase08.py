...
class NNModel(nn.Module):
	...
	def forward(self, x):
		...
		#Fold instead of wrapping the phase
		
		# comnbining the two branches together
...
class CNNTrain():
	...
	def SetInputData(self, fname, add_noise = True):
		...
		# 		self.data['input_data'][i,0,:,:,:] =sm.tsa.acf(self.data['input_data'][i,0,:,:,:]) #only works for 1D arrays! 
			...
			# wrap(array, -1)
			# Returns segmentation fault ; have to be double precision complex arrays 
...
class CNNPredict(CNNTrain):
	...
	def TransferPredict(self, AMP=False):
		if AMP:
...
def Predict():
	...
	predict.SetModel(NNModel, momentum = 0.7, checkpoints=True)
